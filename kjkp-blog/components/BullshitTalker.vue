<template>
	<div id="主体">
		<h3 id="主题">
			<span>主题 </span>
			<input></input>
			<button @click="生成文章()">生成</button>
		</h3>
		<p id="文章" v-html="article"></p>
	</div>
	<div id="声明">
		<p id="声明文字" style="font-size: xx-small; color: aquamarine;">
			<strong>*请勿将生成的文章用于正规用途*</strong>
		</p>
	</div>
	<div id="页脚">
		<p>
			网页版
			<a href="https://github.com/suulnnka/BullshitGenerator"><svg class="图标" viewBox="0 0 16 16" version="1.1"
					aria-hidden="true">
					<path fill-rule="evenodd"
						d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z">
					</path>
				</svg></a> @
			<a href="https://www.zhihu.com/people/su-lan-qia">suulnnka</a>
		</p>
		<p>
			原版
			<a href="https://github.com/menzi11/BullshitGenerator"><svg class="图标" viewBox="0 0 16 16" version="1.1"
					aria-hidden="true">
					<path fill-rule="evenodd"
						d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z">
					</path>
				</svg></a> @
			<a href="https://www.zhihu.com/people/ke-meng-90">menzi11</a>
		</p>
	</div>
</template>
...
export default {
	...
	methods: {
		初始化() {
			this.主题 = this.$route.query.主题 || this.随便取一句(this.初始主题)
			this.随机种子 = this.$route.query.随机种子 || this.下取整(this.随便取一个数(0, this.同余模, Math.random))
			this.$refs.主题.value = this.主题
			this.生成文章()
		},
		下取整(数) {
			return Math.floor(数)
		...
		同余发生器() {
			this.随机种子 = (this.随机种子 * this.同余乘数 + this.同余加数) % this.同余模
			return this.随机种子 / this.同余模
		},
		随便取一句(列表) {
			let 坐标 = this.下取整(this.同余发生器() * 列表.length)
			return 列表[坐标]
		},
		随便取一个数(最小值 = 0, 最大值 = 100, 随机数函数 = this.同余发生器) {
			let 数字 = 随机数函数() * (最大值 - 最小值) + 最小值
			return 数字
		},
		来点名人名言() {
			let 名言 = this.随便取一句(this.名人名言)
			名言 = 名言.replace('曾经说过', this.随便取一句(this.前面垫话))
			名言 = 名言.replace('这不禁令我深思', this.随便取一句(this.后面垫话))
			return 名言
		},
		来点论述() {
			let 句子 = this.随便取一句(this.论述)
			句子 = 句子.replace(RegExp('主题', 'g'), this.主题)
			return 句子
		},
		增加段落(段落) {
			if (段落[段落.length - 1] === ' ') {
		...
		生成文章() {
			...
			this.track('generator', this.主题, this.随机种子)
			let 文章 = []
			let 段落 = '';
			let 文章长度 = 0;
			while (文章长度 < 12000) {
			...
			this.$refs.文章.innerHTML = 排版;
			this.$refs.声明文字.style.display = 'block'
		},
		track(eventName, title, seed) {
